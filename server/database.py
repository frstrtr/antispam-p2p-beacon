...
def remove_spammer_from_db(spammer_id: str) -> bool:
    """Remove a spammer from the database and return success status."""
    conn = sqlite3.connect(DATABASE_FILE)
    cursor = conn.cursor()
    
    # Check if spammer exists first
    cursor.execute("SELECT user_id FROM spammers WHERE user_id = ?", (spammer_id,))
    exists = cursor.fetchone() is not None
    
    if exists:
        cursor.execute("DELETE FROM spammers WHERE user_id = ?", (spammer_id,))
        conn.commit()
        LOGGER.info("Removed spammer %s from database", spammer_id)
    
    conn.close()
    return exists


def get_spammer_from_db(spammer_id: str) -> dict:
    """Get spammer details before removal for logging/audit."""
    conn = sqlite3.connect(DATABASE_FILE)
    cursor = conn.cursor()
    cursor.execute(
        """
        SELECT user_id, lols_bot_data, cas_chat_data, p2p_data, is_spammer, timestamp 
        FROM spammers WHERE user_id = ?
        """,
        (spammer_id,),
    )
    row = cursor.fetchone()
    ...
    if row:
        user_id, lols_bot_data, cas_chat_data, p2p_data, is_spammer, timestamp = row
        return {
            "user_id": user_id,
            "lols_bot_data": lols_bot_data,
            "cas_chat_data": cas_chat_data,
