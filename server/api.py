...
class RemoveIdResource(resource.Resource):
    ...
    def render_POST(self, request: Request):
        ...
        try:
            ...
            LOGGER.debug("%s Using gunban for spammer removal", user_id)
            success = self.p2p_factory.gunban(user_id)
            
            if success:
                LOGGER.info("%s Spammer successfully removed via gunban", user_id)
...
class UnbanResource(resource.Resource):
    """HTTP resource for unbanning spammer IDs."""
    ...
    def render_POST(self, request: Request):
        ...
        try:
            # Get the client IP address for security
            client_ip = request.getClientIP()
            if client_ip != "127.0.0.1":
                LOGGER.warning("Unauthorized unban attempt from IP: %s", client_ip)
                request.setResponseCode(403)  # Forbidden
                return b"Forbidden: Only localhost can access this endpoint."
            ...
            try:
                ...
                user_id = request.args.get(b"user_id", [None])[0]
                if user_id:
                    data = {"spammer_id": user_id.decode("utf-8")}
                else:
                    data = {}
            ...
            spammer_record = get_spammer_from_db(spammer_id)
            if not spammer_record:
                LOGGER.warning("Unban failed: spammer %s not found", spammer_id)
                request.setResponseCode(404)
                return json.dumps({"error": "Spammer not found"}).encode('utf-8')
            
            # Initiate gunban (global unban) through P2P network
            success = self.p2p_factory.gunban(spammer_id)
            ...
            LOGGER.error("Unban endpoint error: %s", e)
