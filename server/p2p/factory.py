...
class P2PFactory(protocol.Factory):
    ...
    def broadcast_user_amnesty(self, user_id):
        ...
        # Deprecated: Use gunban instead
        LOGGER.warning("broadcast_user_amnesty is deprecated, use gunban instead")
        return self.gunban(user_id)

    def propagate_unban(self, spammer_id: str, source_node_uuid: str, exclude=None):
        """Propagate unban to all connected peers except excluded one."""
        # Deprecated: Use gunban instead  
        LOGGER.warning("propagate_unban is deprecated, use gunban instead")
        return self.gunban(spammer_id, source_node_uuid, exclude)
                
    def initiate_unban(self, spammer_id: str):
        """Initiate unban from this node (local request)."""
        # Deprecated: Use gunban instead
        LOGGER.warning("initiate_unban is deprecated, use gunban instead")
        return self.gunban(spammer_id)

    def gunban(self, spammer_id: str, source_node_uuid: str = None, exclude=None):
        """
        Global Unban - Unified method to remove spammer from network.
        
        Args:
            spammer_id: ID of spammer to remove
            source_node_uuid: UUID of originating node (None for local initiation)
            exclude: Protocol instance to exclude from propagation (to prevent loops)
            
        Returns:
            bool: True if successful, False if spammer not found
        """
        from server.database import remove_spammer_from_db
        
        # If no source specified, this is a local initiation
        if source_node_uuid is None:
        ...
        removed = remove_spammer_from_db(spammer_id)
        
        if removed:
            if is_local_request:
                LOGGER.info("GUNBAN: Locally initiated removal of spammer %s", spammer_id)
            else:
                LOGGER.info("GUNBAN: Removed spammer %s via request from node %s", 
                           spammer_id, source_node_uuid)
            
            # Propagate to all connected peers (except the one we received from)
            self._propagate_gunban_message(spammer_id, source_node_uuid, exclude)
            return True
        else:
            if is_local_request:
                LOGGER.warning("GUNBAN: Local removal failed - spammer %s not found", spammer_id)
            else:
                LOGGER.warning("GUNBAN: Propagated removal failed - spammer %s not found", spammer_id)
            return False
    
    def _propagate_gunban_message(self, spammer_id: str, source_node_uuid: str, exclude=None):
        """Internal method to propagate gunban message to peers."""
        message = json.dumps({
            "type": "gunban",
        ...
        propagated_count = 0
        for proto in self.protocol_instances:
            if proto != exclude and hasattr(proto, 'peer_uuid'):
                try:
                    proto.transport.write(message.encode("utf-8"))
                    propagated_count += 1
                    LOGGER.debug("GUNBAN: Sent removal message for %s to peer %s", 
                               spammer_id, proto.peer_uuid)
                except Exception as e:
                    LOGGER.error("GUNBAN: Failed to send to peer %s: %s", 
                               proto.peer_uuid, e)
        
        LOGGER.info("GUNBAN: Propagated removal of %s to %d peers", 
                   spammer_id, propagated_count)
